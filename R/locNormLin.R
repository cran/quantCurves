...
locNormLin<-function(x,y,bandwidth.method="CV", kernel = locpol::gaussK, cents=c(0.03,0.25,0.5,0.75,0.97), disp_window = TRUE, leg = TRUE, axes.lab=NULL){

  # Sequence des x pour lesquels on va faire l'estimation :
  x_seq=seq(min(x),max(x),0.01)

  # Calcul de la fenetre/bande passante :
  bandwidth <- bandwidth(x, y, bandwidth.method)

  # Estimation des parametres par la methode pol loc cte :
  lcw <- locpol::locLinWeightsC(x, x_seq, bandwidth, kernel)$locWeig
  ...
  # "sapply" va ?tre appliquer pour calculer la fonction enchaque centile de "cents"
  ...
  graphics::title(main='Non Parametric linear etimator')
